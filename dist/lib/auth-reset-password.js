...
function buildResetUrlFromActionLink(actionLink) {
  try {
...
async function generateRecoveryLink(email) {
  ...
  return {
    error: formatServiceError(data) || 'Could not create password reset link.',
    status: 502,
  };
}
...
async function verifyRecoveryToken(url, anonKey, token, primaryType) {
  const types = [primaryType || 'recovery', 'magiclink'].filter(function (type, index, arr) {
...
async function deliverPasswordResetEmail(email) {
  ...
  const resetUrl = buildResetUrlFromActionLink(linkResult.link);
