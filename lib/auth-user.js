...
async function reconcileAuthUsersForEmail(cfg, email, preferredUserId) {
  const normalizedEmail = String(email).trim().toLowerCase();
  const users = await listAdminUsersByEmail(cfg, normalizedEmail);
  const preferred = preferredUserId ? await fetchAdminUserById(cfg, preferredUserId) : null;

  if (preferred?.id && !users.some((user) => user.id === preferred.id)) {
    users.push(preferred);
  }

  if (!users.length) {
    return { userId: preferredUserId || null, cleaned: 0 };
  }

  let canonical =
    users.find(
      (user) =>
        user.id === preferredUserId && (user.email || '').trim().toLowerCase() === normalizedEmail
    ) ||
    users.find((user) => (user.email || '').trim().toLowerCase() === normalizedEmail) ||
    users.find((user) => user.id === preferredUserId) ||
    users.find((user) => isUserEmailConfirmed(user)) ||
    users[0];

  let cleaned = 0;
  for (const user of users) {
    if (user.id === canonical.id || isUserEmailConfirmed(user)) continue;
    const deleted = await deleteAdminUser(cfg, user.id);
    if (deleted.ok) cleaned += 1;
  }

  return { userId: canonical.id, cleaned };
}
