...
async function handleAuthConfirm(req, res) {
  ...
  const result = await completeEmailConfirmation(token, type);

  if (!result.ok || !result.access_token || !result.refresh_token) {
    const message = result.error
      ? encodeURIComponent(String(result.error).slice(0, 180))
      : '';
    return res.redirect(
      302,
      message ? `/signin.html?confirm=error&msg=${message}` : '/signin.html?confirm=error'
    );
  }

  const dest = nextPath || '/account.html';
  const hash = new URLSearchParams({
    access_token: result.access_token,
    refresh_token: result.refresh_token,
    type: 'signup',
  });
  if (result.expires_in) hash.set('expires_in', String(result.expires_in));
  if (result.expires_at) hash.set('expires_at', String(result.expires_at));

  return res.redirect(302, `${dest}#${hash.toString()}`);
}

function buildOneClickConfirmUrl(hashedToken, preferredType) {
  const origin = (process.env.AUTH_CONFIRM_ORIGIN || 'https://gaviom.com').replace(/\/$/, '');
  const params = new URLSearchParams({
    token: String(hashedToken || '').trim(),
