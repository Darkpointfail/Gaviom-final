...
async function issueVerificationCodeForUser(userId, email, meta) {
  ...
  const reconciled = await reconcileAuthUsersForEmail(cfg, emailCheck.email, resolvedUserId);
  resolvedUserId = reconciled.userId || resolvedUserId;
  if (reconciled.cleaned > 0) {
    console.info('email-verification:cleaned-duplicate-users', {
      email: emailCheck.email,
      keptUserId: resolvedUserId,
...
async function verifyEmailCode(email, code, meta) {
  ...
  const reconciled = await reconcileAuthUsersForEmail(cfg, emailCheck.email, row.user_id || adminUser.id);
  const targetUserId = reconciled.userId || row.user_id || adminUser.id;
  if (reconciled.cleaned > 0) {
    console.info('email-verification:cleaned-duplicate-users', {
      email: emailCheck.email,
      keptUserId: targetUserId,
      cleaned: reconciled.cleaned,
    });
  }
  if (targetUserId && row.user_id && targetUserId !== row.user_id) {
    await repointVerificationCodeUserId(cfg, row.id, targetUserId);
  }

  const targetUser = (await fetchAdminUserById(cfg, targetUserId)) || adminUser;
  if (isUserEmailConfirmed(targetUser)) {
    ...
    return {
      ...
      email: emailCheck.email,
      email_confirmed_at: targetUser.email_confirmed_at || targetUser.confirmed_at,
    };
  }

  const confirmResult = await confirmSupabaseUser(cfg, targetUserId);
  if (!confirmResult.ok) {
  ...
  const refreshedUser = confirmResult.user || (await fetchAdminUserById(cfg, targetUserId));
