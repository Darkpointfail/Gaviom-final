...
async function confirmSupabaseUser(cfg, userId) {
  ...
  if (!res.ok) {
    console.error('email-verification:confirm-user', res.status, data?.msg || data?.message || data);
    return { ok: false, user: null };
  }
  return { ok: true, user: data?.id ? data : null };
}
...
async function verifyEmailCode(email, code, meta) {
  ...
  const confirmResult = await confirmSupabaseUser(cfg, adminUser.id);
  if (!confirmResult.ok) {
    return { error: 'Could not confirm your email. Try again.', status: 502 };
  }

  const refreshedUser = (await fetchAdminUserByEmail(cfg, emailCheck.email)) || confirmResult.user;
  if (!refreshedUser?.email_confirmed_at) {
    return { error: 'Email confirmation did not complete. Try again.', status: 502 };
  }
