...
async function completeEmailConfirmationByProof(email, proof) {
  ...
  let session = await issueSessionForEmail(cfg, url, anonKey, parsed.email);
  if (session?.data?.access_token) {
    ...
    const confirmed =
      isUserEmailConfirmed(freshAdminUser) || isUserEmailConfirmed(canonicalUser) || isUserEmailConfirmed(session.data.user);
    if (confirmed) {
      return {
        ok: true,
  ...
  session = await issueSessionForEmail(cfg, url, anonKey, parsed.email);
  if (!session?.data?.access_token) {
    return {
  ...
  const canonicalUser = mergeCanonicalUser(session.data.user, confirmResult.user);
