...
async function completeEmailConfirmationByProof(email, proof) {
  ...
  let session = await confirmViaGeneratedLink(cfg, authCfg.url, authCfg.anonKey, parsed.email);
  if (session?.data?.access_token) {
    ...
    let freshAdminUser = await fetchAdminUserById(cfg, userId);
    if (!freshAdminUser || !isUserEmailConfirmed(freshAdminUser)) {
      await ensureUserEmailConfirmed(cfg, userId, parsed.email);
      freshAdminUser = await fetchAdminUserById(cfg, userId);
    }
    return buildSessionSuccess(
      Object.assign({}, session.data, { user: tokenUser || session.data.user }),
      freshAdminUser
    );
  }

  const confirmResult = await ensureUserEmailConfirmed(cfg, userId, parsed.email);
  if (confirmResult.session?.access_token) {
    const freshAdminUser = confirmResult.user || (await fetchAdminUserById(cfg, userId));
    return buildSessionSuccess(confirmResult.session, freshAdminUser);
  }

  if (!confirmResult.ok) {
    console.error('auth-confirm-proof:confirm-failed', { userId, email: parsed.email });
    return {
  ...
  session = await confirmViaGeneratedLink(cfg, authCfg.url, authCfg.anonKey, parsed.email);
  if (!session?.data?.access_token) {
    return {
  ...
  return buildSessionSuccess(session.data, confirmResult.user);
}
