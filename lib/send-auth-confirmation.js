...
const { buildConfirmUrlFromActionLink } = require('./auth-confirm-email');

const FROM = (process.env.AUTH_CONFIRM_FROM || 'Gaviom <noreply@getgaviom.com>').trim();
const REDIRECT = (process.env.AUTH_CONFIRM_REDIRECT || 'https://gaviom.com/account.html').trim();
...
async function deliverAuthConfirmationEmail(email) {
  ...
  const confirmUrl = buildConfirmUrlFromActionLink(linkResult.link, linkResult.type);
