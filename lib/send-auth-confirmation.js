...
const { buildOneClickConfirmUrl, extractLinkProperties } = require('./auth-confirm-email');
...
async function generateConfirmationLink(email) {
  ...
  const types = ['signup', 'invite', 'magiclink'];
  let lastError = null;
...
<html><body style="font-family:system-ui,sans-serif;line-height:1.6;color:#0a1628;max-width:560px">
  ...
  <p style="color:#64748b;font-size:14px">One click on the button above confirms your account and signs you in.</p>
...
async function deliverAuthConfirmationEmail(email) {
  ...
  const confirmUrl = buildOneClickConfirmUrl(
