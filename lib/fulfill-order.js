...
async function insertEntries({ userId, email, items, orderId, source }) {
  ...
  for (const item of items) {
    let creatorSweepstakesId = null;
    const prizeId = item.prizeId;
    if (prizeId && String(prizeId).startsWith('cr-')) {
      const params = new URLSearchParams({
        select: 'id',
        prize_id: `eq.${prizeId}`,
        limit: '1',
      });
      const lookup = await adminFetch(`creator_sweepstakes?${params}`);
      const match = Array.isArray(lookup.data) ? lookup.data[0] : null;
      if (match && match.id) creatorSweepstakesId = match.id;
    }
    ...
    rows.push({
      ...
      creator_sweepstakes_id: creatorSweepstakesId,
    });
  }
